...
from harness.validators import Validators, ValidationStatus, run_all_validators
...
def test_run_all_validators():
    ...
    for r in results:
        status_icon = "✅" if r.status == ValidationStatus.PASSED else ("⚠️" if r.status == ValidationStatus.WARNING else "❌")
