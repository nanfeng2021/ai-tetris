...
class AIAgent:
        ...
        for x in range(self.board.width):
            ...
            for y in range(self.board.height):
                if board[y][x] is None:
                    ...
                    right_blocked = x == self.board.width - 1 or (y < self.board.height and board[y][x+1] is not None)
    ...
    def _calculate_landing_height(self, board: List[List[str]], piece_x: int) -> int:
        """计算着陆高度"""
        for y in range(self.board.height):
            if any(board[y][x] is not None for x in range(piece_x, min(piece_x + 4, self.board.width))):
