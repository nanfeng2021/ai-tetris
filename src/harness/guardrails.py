...
class Guardrails:
    ...
    @staticmethod
    def validate_boundary(x: int, y: int, piece_width: int, piece_height: int, 
                         board_width: int = 10, board_height: int = 20) -> GuardrailResult:
        """边界检查：方块不能穿墙"""
    ...
    @staticmethod
    def detect_stuck(last_move_time: float, current_time: float, timeout: float = 5.0) -> GuardrailResult:
        """卡死检测：超过 5 秒无有效移动"""
...
def run_all_guardrails(**kwargs) -> List[GuardrailResult]:
    ...
    if 'x' in kwargs and 'y' in kwargs:
        results.append(Guardrails.validate_boundary(
            kwargs['x'], kwargs['y'],
    ...
    if 'board' in kwargs and 'piece' in kwargs:
        results.append(Guardrails.validate_collision(
            kwargs['board'], kwargs['piece'],
            kwargs['x'], kwargs['y']
        ))
    
    if 'score' in kwargs:
        results.append(Guardrails.validate_game_state(
            kwargs.get('score', 0),
            kwargs.get('lines', 0),
            kwargs.get('level', 1)
        ))
