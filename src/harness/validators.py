...
class Validators:
    ...
    def validate_board_dimensions(
        board: List[List[str]], expected_width: int = 10, expected_height: int = 20
    ) -> ValidationResult:
        """验证面板尺寸"""
    ...
    @staticmethod
    def validate_game_progress(
        score: int, lines: int, level: int, elapsed_time: float
    ) -> ValidationResult:
        """验证游戏进度合理性"""
        # 分数不能为负
        if score < 0:
...
def run_all_validators(**kwargs) -> List[ValidationResult]:
    ...
    if "score" in kwargs and "lines" in kwargs and "level" in kwargs:
        results.append(
            Validators.validate_game_progress(
                kwargs["score"],
                kwargs["lines"],
                kwargs["level"],
